...
* Для отступов всегда используем для отступа два/три/четыре пробела. Не используем табуляцию.
...
* желательно комментировать все функции, классы, миксины BlockDoc.
...
1. Ключевые (без которых тег не имеет смысла) для атрибутов (кроме src, этот последний)
2. ID и Class
3. все остальные
...
* Верстку начинать с мобильной версии (особенно если сайт для пользователей)
...
* Атомарные стили прописать в base, layout и переносить их за собой от проекта к проекту
...
* заботится об утечках памяти. (это не значит что надо 1 объект/переменную на все случаи жизни)
...
* html2pdf - формирование pdf для php <5.6
...
* нормальный текстовый редактор/IDE - желательно что от jetbrains
...
* babel - по особым требованиям (если клиент требует поддержку старых брузеров, кроме IE)
